...
// Helper function to get user's watchlist symbols
async function getUserWatchlistSymbols(): Promise<Set<string>> {
  try {
    ...
    return new Set(items.map((item) => String(item.symbol).toUpperCase()));
  } catch (err) {
      ...
      const mapped: StockWithWatchlistStatus[] = results
        .map((r) => {
          ...
          const item: StockWithWatchlistStatus = {
            ...
            exchange,
            type,
            isInWatchlist: watchlistSymbols.has(upper), // Check against watchlist
          };
          return item;
        })
        .slice(0, 15);
